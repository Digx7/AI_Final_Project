...
def my_custom_node_sort(obj):
    order = {'FIGHT': 0, 'DRAW_HAMSTER': 1, 'DRAW_CARD': 2}
    return order.get(obj.action, 2)


def my_best_first_graph_search(problem, display=False):
    ...
    while frontier:
        # print("\nStarting while loop")
        frontier = sorted(frontier, key=my_custom_node_sort)
        node = frontier.pop()

        # print(f'Looking at node with action {node.action}')

        if node.action:
            if node.action is 'FIGHT':
                if problem.goal_test(node.state):
                    print("GOAL FOUND")
                    if display:
                        print(len(explored), "paths have been expanded and", len(frontier), "paths remain in the frontier")
        ...
        for child in node.expand(problem):
            ...
            # print(f'Looking at child node with action {child.action}')
            # print("Child node state")
            # child.state.print()
            ...
            if child not in explored and child not in frontier:
                ...
                # print("child is already in the frontier")

                # print(frontier)
                # print(child)

                index = frontier.index(child)
                if child.state.enemy_health < frontier[index].state.enemy_health:
                    del frontier[index]
                    frontier.append(child)
                    # print("REPLACING node in frontier")
            # elif child in explored:
                # print("child has already been explored")

        # print(len(explored), "paths have been expanded and", len(frontier), "paths remain in the frontier")

        if time.time() - last_print_time > interval:
            print('\r', ' ' * 1, end='', flush=True)
            print(len(explored), "paths have been expanded and", len(frontier), "paths remain in the frontier", end='',
                  flush=True)
